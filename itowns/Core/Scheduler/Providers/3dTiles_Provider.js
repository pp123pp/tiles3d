...
function configureTile(tile, layer, metadata, parent) {
    ...
    // parse metadata（解析tileset.json原数据）
    //获取json数据中的transform矩阵数据
    ...
    //geometricError：与计算的SSE(屏幕空间误差)值相比较的阈值
    tile.geometricError = metadata.geometricError;
    //当前瓦片的id
    tile.tileId = metadata.tileId;
    //父子瓦片的替换规则，add，还是replace
    ...
    //metadata.boundingVolume：包围当前瓦片的包围体
    //获取当前包围盒
...
$3dTiles_Provider.prototype.executeCommand = function executeCommand(command) {
    ...
    if (path) {
        ...
        return Fetcher.arrayBuffer(url, layer.networkOptions).then((result) => {
            ...
            if (result !== undefined) {
                let func;
                //textDecoder.decode:根据指定的解码方式对编码数据进行解码，并返回解码后的字符串数据
                const magic = textDecoder.decode(new Uint8Array(result, 0, 4));
                //根据解码之后的字符串，选择对应的加载方式
                if (magic[0] === '{') {
